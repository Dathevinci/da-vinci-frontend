...
export default async function ExplorePage({
  ...
  const titleParam = resolvedParams.title || "Explore Anime";
  ...
  let media: any[] = [];
  let pageInfo: any = { total: 0, hasNextPage: false };

  try {
    const results = await searchAnime(variables);
    media = results?.Page?.media || [];
    pageInfo = results?.Page?.pageInfo || { total: 0, hasNextPage: false };
  } catch (err) {
      ...
      <div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          <div className="mb-6 flex justify-between items-end border-b border-white/10 pb-4">
            <div>
              <h1 className="text-3xl font-black tracking-tight text-white drop-shadow-md">{titleParam}</h1>
            ...
            <div className="hidden sm:flex gap-2 text-sm font-bold">
              ...
              {pageInfo.hasNextPage && (
                <Link href={createPageUrl(page + 1)} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 border border-purple-500/50 rounded-lg transition shadow-lg flex items-center gap-1 text-white">
          ...
          {media.length > 0 && (
            <div className="mt-12 flex justify-center gap-4 text-sm font-bold">
              ...
              {pageInfo.hasNextPage ? (
                <Link href={createPageUrl(page + 1)} className="px-6 py-3 bg-purple-600 hover:bg-purple-500 border border-purple-500/50 rounded-xl transition shadow-lg flex items-center gap-2 text-white">
