...
import { Compass, Eye, Heart, Clock, Check, ListFilter, Settings, Camera, UploadCloud, AlertCircle } from "lucide-react";
  ...
  return (
    <div className="bg-[#09090b] min-h-screen pt-24 pb-12 px-4 md:px-12 text-white">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Profile Header */}
        {user && (
          <div className="flex flex-col md:flex-row items-center gap-6 bg-white/5 border border-white/10 p-8 rounded-3xl mb-10 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/10 blur-3xl rounded-full -translate-y-1/2 translate-x-1/4"></div>
            
            <div className="relative group cursor-pointer" onClick={() => activeTab === "Settings" && fileInputRef.current?.click()}>
              {user.avatar ? (
                <img src={user.avatar} alt="Avatar" className="w-28 h-28 rounded-full object-cover border-4 border-indigo-500 shadow-xl" />
              ) : (
                <div className="w-28 h-28 bg-indigo-600 rounded-full flex items-center justify-center text-4xl font-black border-4 border-indigo-500 shadow-xl">
            ...
            <div className="text-center md:text-left z-10">
              ...
              <p className="text-indigo-300 font-medium">{user.bio || "No bio set. Update your settings to add one!"}</p>
            </div>
          </div>
        )}
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-2 bg-indigo-600/20 hover:bg-indigo-600/40 border border-indigo-500/30 text-indigo-300 px-4 py-2 rounded-lg font-bold transition disabled:opacity-50"
                  >
