...
export default class FlameComics extends MangaParser {
  ...
  protected override readonly baseUrl = 'https://flamecomics.com';
  protected override readonly logo = 'https://flamecomics.com/wp-content/uploads/2021/03/Flame-Scans-Logo-1.png';
  ...
  override async fetchMangaInfo(mangaId: string): Promise<IMangaInfo> {
    const slug = mangaId.replace('flc:', '');
    ...
    try {
      ...
      $('#chapterlist li').each((_, el) => {
        ...
        if (cHref) {
          // e.g. https://flamecomics.com/series/slug/chapter-1/
          const cSlugMatch = /\/series\/[^/]+\/([^/]+)/.exec(cHref);
          if (cSlugMatch) {
            chapters.push({
              id: `flc:${slug}|${cSlugMatch[1]}`,
              title: cTitle,
              releaseDate: cDate,
            });
          }
      ...
      console.error(`FlameComics fetchMangaInfo error for ${slug}:`, e);
      throw e;
    }
  }
