...
import { Settings, LogOut, Shield, Bug, Search, Moon, Sun, Lock, Unlock, Zap, PlayCircle, EyeOff } from "lucide-react";
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed top-20 right-4 md:right-8 w-[calc(100vw-32px)] sm:w-96 bg-black/70 backdrop-blur-3xl border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.8)] rounded-3xl p-4 z-[100] flex flex-col gap-4 text-white overflow-hidden max-h-[85vh] overflow-y-auto"
        >
          {/* Quick Toggles Grid */}
          <div className="grid grid-cols-2 gap-3">
            ...
            <button 
              onClick={toggleTheme}
              className={`flex flex-col p-4 rounded-2xl transition-colors ${theme === 'dark' ? 'bg-purple-600/90 hover:bg-purple-500' : 'bg-white/10 hover:bg-white/20'}`}
            >
              ...
              <span className="text-[10px] text-white/70 text-left">{theme === 'dark' ? 'Neon Purple' : 'Pure Black'}</span>
            </button>
          ...
          {/* Advanced Settings Link removed */}
        </motion.div>
      )}
    </AnimatePresence>
  );
